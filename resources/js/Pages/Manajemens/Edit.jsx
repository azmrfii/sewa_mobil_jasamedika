...
export default function TambahMobil({ auth }) {
    const { manajemen } = usePage().props
    const { data, setData, patch, errors, processing, recentlySuccessful } = useForm({
        merek: manajemen.merek || "",
        model: manajemen.model || "",
        nomor_plat: manajemen.nomor_plat || "",
        tarif_sewa: manajemen.tarif_sewa || ""
    });
  
    function handleSubmit(e) {
        e.preventDefault();
        patch(route("manajemens.update", manajemen.id));
    }
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Ubah Data Mobil</h2>}
        >
            <Head title="Ubah Data Mobil" />
